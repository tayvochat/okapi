...
use tayvo_rocket_okapi::response::OpenApiResponderInner;
use tayvo_rocket_okapi::tayvo_okapi;
use tayvo_rocket_okapi::tayvo_okapi::openapi3::{MediaType, Responses};
use tayvo_rocket_okapi::tayvo_okapi::schemars;
use tayvo_rocket_okapi::settings::UrlObject;
use tayvo_rocket_okapi::{openapi_get_routes, rapidoc::*, swagger_ui::*, OpenApiError};
...
pub fn bad_request_response(gen: &mut OpenApiGenerator) -> tayvo_okapi::openapi3::Response {
    let schema = gen.json_schema::<MyError>();
    tayvo_okapi::openapi3::Response {
        description: "\
        # 400 Bad Request\n\
        The request given is wrongly formatted or data was missing. \
        "
        .to_owned(),
        content: tayvo_okapi::map! {
...
pub fn unauthorized_response(gen: &mut OpenApiGenerator) -> tayvo_okapi::openapi3::Response {
    let schema = gen.json_schema::<MyError>();
    tayvo_okapi::openapi3::Response {
        description: "\
        # 401 Unauthorized\n\
...
impl OpenApiResponderInner for MyError {
    fn responses(gen: &mut OpenApiGenerator) -> Result<Responses, OpenApiError> {
        use tayvo_okapi::openapi3::RefOr;
        Ok(Responses {
            responses: tayvo_okapi::map! {
