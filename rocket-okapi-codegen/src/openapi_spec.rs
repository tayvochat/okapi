...
pub fn create_openapi_spec(routes: TokenStream) -> Result<TokenStream2> {
    ...
    Ok(quote! {
        |settings: &::tayvo_rocket_okapi::settings::OpenApiSettings| -> ::tayvo_rocket_okapi::tayvo_okapi::openapi3::OpenApi {
            let mut gen = ::tayvo_rocket_okapi::gen::OpenApiGenerator::new(settings);
            #add_operations
            let mut spec = gen.into_openapi();
            let mut info = ::tayvo_rocket_okapi::tayvo_okapi::openapi3::Info {
            ...
            if !env!("CARGO_PKG_REPOSITORY").is_empty() {
                info.contact = Some(::tayvo_rocket_okapi::tayvo_okapi::openapi3::Contact{
