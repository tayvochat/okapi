...
pub fn parse_routes(routes: TokenStream) -> Result<TokenStream2> {
    ...
    Ok(quote! {
        |spec_opt: Option<::tayvo_rocket_okapi::tayvo_okapi::openapi3::OpenApi>, settings: &::tayvo_rocket_okapi::settings::OpenApiSettings|
            -> Vec<::rocket::Route> {
                let mut routes = ::rocket::routes![#paths];
                if let Some(spec) = spec_opt {
                    routes.push(
                        ::tayvo_rocket_okapi::handlers::OpenApiHandler::new(spec)
