...
//! tayvo_rocket_okapi = { version = "0.9.1", features = ["swagger"] }
//! ```
...
//! use tayvo_rocket_okapi::{openapi, openapi_get_routes, JsonSchema};
//! use tayvo_rocket_okapi::swagger_ui::{make_swagger_ui, SwaggerUIConfig};
//!
...
//!     use tayvo_rocket_okapi::settings::UrlObject;
//!
//!     SwaggerUIConfig {
...
/// Re-export Okapi
pub use tayvo_okapi;
pub use tayvo_rocket_okapi_codegen::*;
...
pub struct OperationInfo {
    ...
    pub operation: tayvo_okapi::openapi3::Operation,
}
...
pub fn get_openapi_route(
    spec: tayvo_okapi::openapi3::OpenApi,
...
macro_rules! mount_endpoints_and_merged_docs {
    ($rocket_builder:ident, $base_path:expr, $openapi_settings:ident,
     $($path:expr => $route_and_docs:expr),* $(,)*) => {{
        ...
        let mut openapi_list: Vec<(_, tayvo_rocket_okapi::tayvo_okapi::openapi3::OpenApi)> = Vec::new();
        $({
        ...
        let openapi_docs = match tayvo_rocket_okapi::tayvo_okapi::merge::marge_spec_list(&openapi_list){
            Ok(docs) => docs,
        ...
        $rocket_builder = $rocket_builder.mount(
            $base_path,
            vec![tayvo_rocket_okapi::get_openapi_route(
                openapi_docs,
                &$openapi_settings,
            )],
        );
...
/// Example:
/// ```rust,ignore
/// use tayvo_okapi::openapi3::OpenApi;
/// let settings = tayvo_rocket_okapi::settings::OpenApiSettings::new();
...
macro_rules! openapi_get_routes {
    ...
    ($settings:ident :
     $($route:expr),* $(,)*) => {{
        let spec = tayvo_rocket_okapi::openapi_spec![$($route),*](&$settings);
        let routes = tayvo_rocket_okapi::openapi_routes![$($route),*](Some(spec), &$settings);
        routes
    }};
    ...
    ($($route:expr),* $(,)*) => {{
        let settings = tayvo_rocket_okapi::settings::OpenApiSettings::new();
        tayvo_rocket_okapi::openapi_get_routes![settings: $($route),*]
    }};
}
...
macro_rules! openapi_get_routes_spec {
    ...
    ($settings:ident :
     $($route:expr),* $(,)*) => {{
        let spec = tayvo_rocket_okapi::openapi_spec![$($route),*](&$settings);
        let routes = tayvo_rocket_okapi::openapi_routes![$($route),*](None, &$settings);
        (routes, spec)
    }};
    ...
    ($($route:expr),* $(,)*) => {{
        let settings = tayvo_rocket_okapi::settings::OpenApiSettings::new();
        tayvo_rocket_okapi::openapi_get_routes_spec![settings: $($route),*]
    }};
}
...
macro_rules! openapi_get_spec {
    ...
    ($($route:expr),* $(,)*) => {{
        let settings = tayvo_rocket_okapi::settings::OpenApiSettings::new();
        tayvo_rocket_okapi::openapi_get_spec![settings: $($route),*]
    }};
}
